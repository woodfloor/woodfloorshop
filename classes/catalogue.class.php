...
class Catalogue {
	...
	public function displayCategory() {
		...
		if (!empty($products)) {
			foreach ($products as $product) {
				...
				$this->productAssign($product, false, true);
	...
	public function displayProduct($product = false, $popularity = false) {
		if (isset($product) && is_numeric($product)) {
			if (($product = $this->getProductData($product)) !== false) {
				...
				$this->productAssign($product, true, true);
	...
	public function getProductPrice(&$product_data, $quantity = 1) {
		if (isset($product_data['product_id']) && is_numeric($product_data['product_id'])) {
			...
			$packm2 = $GLOBALS['db']->select('CubeCart_inventory', 'calc_packm2', array('product_id' => $product_id));

			if($packm2 && $packm2[0]['calc_packm2'] > 0) {	 
                $product_data['calc_packm2'] = $packm2[0]['calc_packm2'];	 	 
                $product_data['price'] = $product_data['price'] * $packm2[0]['calc_packm2']; 	 
                $product_data['sale_price'] = $product_data['sale_price'] * $packm2[0]['calc_packm2'];	 	 
            } else {	 	 
                $product_data['calc_packm2'] = 1;	 	 
            }
			...
			if (($pricing = $GLOBALS['db']->select('CubeCart_pricing_quantity', array('quantity', 'price'), $search, array('quantity' => 'ASC', 'price' => 'ASC'))) !== false) {
				foreach ($pricing as $price) {
					$tmp_price = ($GLOBALS['config']->get('config', 'catalogue_sale_mode')==2) ? ($price['price'] - ($price['price'] / 100) * $GLOBALS['config']->get('config', 'catalogue_sale_percentage')) : $price['price'];
					if($product_data['calc_packm2']>0) {
                    	$prices[$price['quantity']] = $product_data['calc_packm2'] * $tmp_price;	 	 
                    }
				}
				krsort($prices);
	 ...
	 */
	public function productAssign(&$product, $product_view = true, $m2 = false) {
		## Short Description
		...
		$m2_amount = ($m2) ? $product['calc_packm2'] : 1;

		$product['price_unformatted']  = $product['price']/$m2_amount;
		$product['sale_price_unformatted'] = $product['sale_price']/$m2_amount;

		$product['price']  = $GLOBALS['tax']->priceFormat($product['price']/$m2_amount);
		$product['sale_price'] = $GLOBALS['tax']->priceFormat($product['sale_price']/$m2_amount);
		...
		if ($product_view) {
			...
			if (($prices = $GLOBALS['db']->select('CubeCart_pricing_quantity', false, array('product_id' => $product['product_id'], 'group_id' => $group_id), array('quantity' => 'ASC'))) !== false) {
				...
				foreach ($prices as $price) {
					...
					if($GLOBALS['session']->get('prices_with_and_without_tax')) {
						$tax_data = $GLOBALS['tax']->fetchTaxDetails($product['tax_type']);
                        $price['price'] = $price['price'] + ($price['price'] * ($tax_data['tax_percent']/100));
                    }

					if($product['calc_packm2']>0) {	 
                        $price['price'] = $product['calc_packm2'] * $price['price'];	 	 
                    }
					$price['price_m2'] = $GLOBALS['tax']->priceFormat(($price['price']/$m2_amount), true);
					$price['quantity_m2'] = $price['quantity']*$m2_amount;
				...
				if ($search_mode == 'fulltext' && $search_str_len >= $max_word_len) {
					...
					if ($search = $GLOBALS['db']->query($query)) {
						...
						foreach($search as $item) {
							$item['price'] = $item['price'] * $item['calc_packm2'];
							$item['sale_price'] = $item['sale_price'] * $item['calc_packm2'];
							if($item['price']>$item['sale_price'] && $item['sale_price']>0) {
								$item['ctrl_sale'] = true;
							}
							$output[] = $item;
						}

						$this->_category_products = $output;
