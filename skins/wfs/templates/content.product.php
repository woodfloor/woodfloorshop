      ...
      <div class="row">
         ...
         <div class="small-7 medium-5 columns">
            ...
            <h3 class="text-right">
               ...
               <span class="old_price" id="fbp" data-price="{$PRODUCT.full_base_price}">{if $PRODUCT.price_m2}{$PRODUCT.price_m2}{else}{$PRODUCT.price}{/if}</span>
               <span class="sale_price" id="ptp" data-price="{$PRODUCT.price_to_pay}">{if $PRODUCT.sale_price_m2}{$PRODUCT.sale_price_m2}{else}{$PRODUCT.sale_price}{/if}</span>
               {if $PRODUCT.calc_status==1} per m<sup>2</sup>{/if}
               {else}
				{if $PRODUCT.price_unformatted == '0.00'}
					<span>P.O.A</span>
				{else}
               {* {$PRODUCT.price_to_pay} changed to {$PRODUCT.price|regex_replace:"/[^0-9.]*/":""} *}
					<span id="ptp" data-price="{$PRODUCT.price|regex_replace:"/[^0-9.]*/":""}">{$PRODUCT.price}</span>
               {if $PRODUCT.calc_status==1} per m<sup>2</sup>{/if}
				{/if}
               {/if}
            ...
            <h3>{$discount.quantity_m2}m<sup>2</sup> +<span class="right"><span class="sale_price price_per_pack_discount" data-quantity="{$discount.quantity}" data-price="{$discount.price_raw}">{$discount.price_m2}</span>{if $PRODUCT.calc_status==1} per m<sup>2</sup>{/if}</span></h3>
            {/foreach}

            {* if isset($PRODUCT.discounts)}
            <p>(<a href="#quantity_discounts">{$LANG.catalogue.bulk_discount}</a>)</p>
            {/if *}
            {if $PRODUCT.calc_status==1}
            <div class="row">
               <div class="small-12 columns">
               Pack size {$PRODUCT.calc_packm2}m<sup>2</sup><br>&pound;<span id="price_per_pack">{($PRODUCT.price_to_pay*$PRODUCT.calc_packm2)|string_format:"%.2f"}</span> per pack

               <div class="hide" id="free_underlay_area">{$CC_SQM_CALC.free_underlay_area}</div>
               <div class="hide" id="free_underlay_group">{$CC_SQM_CALC.free_underlay_group}</div>
               </div>
            </div>
            <div class="row">
               <div class="panel callout thinpad">
                  <div class="small-12 columns nopad">
                     <strong>To calculate number of packs needed Add your floor area</strong>
                  </div> 
                  {*
                  <div class="row">
                     <div class="small-6 columns">Length (m):</div>
                     <div class="small-6 columns">Width (m):</div>
                  </div>
                  <div class="row">
                     <div class="small-6 columns"><input type="text" min="0" class="calc_m2" id="length"></div>
                     <div class="small-6 columns"><input type="text" min="0" class="calc_m2" id="width"></div>
                  </div>
                  *}
                  <div class="row">
                     <div class="small-5 columns">Floor Area</div>
                     <div class="small-4 columns"><input type="text" min="0" class="calc_m2" id="area"></div>
                     <div class="small-3 columns">m<sup>2</sup></div>
                  </div>
                  <div class="row">
                     <div class="small-12 columns">
                     <a href="#" data-reveal-id="how_to_measure">{$HOW_TO.doc_name} <i class="fa fa-info-circle"></i></a>
                     <div id="how_to_measure" class="reveal-modal" data-reveal aria-labelledby="modalTitle" aria-hidden="true" role="dialog">
                       <h3>{$HOW_TO.doc_name}</h3>
                       {$HOW_TO.doc_content}
                  ...
                  <div class="row pad-top">
                     <div class="small-12 columns"><input type="button" class="button secondary tiny expand" value="Calculate Now" id="calc_now"></div>
                  </div>
                  *}
                  <div class="row hide" id="how_many">
                     <div class="small-12 columns">
                        <input type="hidden" id="packm2" value="{$PRODUCT.calc_packm2}">
                        You need <span id="no_packs">0</span> packs (<span id="total_m2">0</span>m<sup>2</sup>)
                        <span class="hide discount_applied">discount applied &pound;<span id="price_per_pack_discount"></span> per pack</span>
                        <br><span class="right hide" id="free_underlay_dia"><a href="#" data-reveal-id="free_underlay">{$UNDERLAY.doc_name} <i class="fa fa-info-circle"></i></a></span>&pound;<span id="total_price">0</span>

                        <div id="free_underlay" class="reveal-modal" data-reveal aria-labelledby="modalTitle" aria-hidden="true" role="dialog">
                        <h3>{$UNDERLAY.doc_name}</h3>
                           {$UNDERLAY.doc_content}
                           <a class="close-reveal-modal" aria-label="Close">&#215;</a>
                        </div>
                     </div>
                  </div>
            ...
            <div class="row" id="option_block_{$option.option_id}">
               <div class="small-12 columns">
            ...
            <div class="row" id="option_block_{$option.option_id}">
               <div class="small-12 columns">
                  ...
                  <select name="productOptions[{$option.option_id}]" id="productOptions[{$option.option_id}]" class="nomarg" {if $option.required}required{/if}>
                    <option value="">{$LANG.form.please_select}</option>
                    
                  {foreach from=$option.values item=value}
                  
                    <option value="{$value.assign_id}" data-price="{$value.decimal_price}" id="{$value.value_name}">{$value.value_name}{if $value.price>0} {$value.symbol}{$value.price}{/if}</option>
            ...
            <div  class="small-12 medium-10 columns">
               ...
               {include file='templates/element.addon_products.php'}
               {/if}
            </div>
            {else}
            {if $CTRL_HIDE_PRICES}
